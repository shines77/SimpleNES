...
void CPU::interruptSequence(InterruptType type)
{
    ...
    Byte flags = Byte(f_N) << 7 |
                 Byte(f_V) << 6 |
                   Byte(1) << 5 |   // unused bit, supposed to be always 1
        Byte(type == BRK_) << 4 |   // B flag set if BRK
                 Byte(f_D) << 3 |
                 Byte(f_I) << 2 |
                 Byte(f_Z) << 1 |
                 Byte(f_C);
        ...
        case PHP:
            {
                Byte flags = Byte(f_N) << 7 |
                             Byte(f_V) << 6 |
                               Byte(1) << 5 |    // supposed to always be 1
                               Byte(1) << 4 |    // PHP pushes with the B flag as 1, no matter what
                             Byte(f_D) << 3 |
                             Byte(f_I) << 2 |
                             Byte(f_Z) << 1 |
                             Byte(f_C);
...
bool CPU::executeBranch(Byte opcode)
{
    if ((opcode & BranchInstructionMask) == BranchInstructionMaskResult) {
        ...
        bool branch = ((opcode & BranchConditionMask) != 0);
        ...
        switch (op)
        {
            ...
            case ROL:
                ...
                else
                {
                    ...
                    setZN(Byte(operand));
                    m_bus.write(location, Byte(operand));
                }
                break;
