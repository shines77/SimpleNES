            ...
            if (m_cycle > 0 && m_cycle <= ScanlineVisibleDots) {
                ...
                if (m_showSprites && (!m_hideEdgeSprites || x >= 8)) {
                    for (auto i : m_scanlineSprites) {
                        ...
                        // Exit the loop now since we've found the highest priority sprite
                        break;
                    }
                }
            ...
            if (m_cycle >= ScanlineEndCycle) {
                ...
                for (std::size_t i = m_spriteDataAddress / 4; i < 64; ++i) {
                    ...
                    if (0 <= diff && diff < range) {
                        ...
                        m_scanlineSprites.push_back(sf::Uint8(i));
                        ++j;
                    }
                }
...
{
    m_generateInterrupt = ((ctrl & 0x80) != 0);
    m_longSprites = ((ctrl & 0x20) != 0);
...
{
    m_greyscaleMode = ((mask & 0x1) != 0);
    m_hideEdgeBackground = !((mask & 0x2) != 0);
    m_hideEdgeSprites = !((mask & 0x4) != 0);
    m_showBackground = ((mask & 0x8) != 0);
    m_showSprites = ((mask & 0x10) != 0);
}
